...
namespace DepthViewer.Utils
{
        ...
        , IDownloadCache
    {
        ...
        public CustomMvxFileDownloadCache(string cacheName, string cacheFolder, int maxFileCount, TimeSpan maxFileAge)
        {
        ...
        private void DoRequestLocalFilePath(string httpSource, Action<string> success, Action<Exception> error)
        {
            RunSyncOrAsyncWithLock(() =>
            {
                ...
                var fileService = MvxFileStoreHelper.SafeGetFileStore();
                var pathForDownload = fileService.PathCombine(_cacheFolder, Guid.NewGuid().ToString("N")) + Path.GetExtension(httpSource);
